...
const tabMap = { '/home': 'home', '/orders': 'orders', '/reservations': 'reservations', '/records': 'records', '/mine': 'mine' }
watch(() => route.path, (p) => {
  for (const [k, v] of Object.entries(tabMap)) { if (p.startsWith(k)) { activeTab.value = v; break } }
}, { immediate: true })

function onTabChange(tab) {
  const paths = { home: '/home', orders: '/orders', reservations: '/reservations', records: '/records', mine: '/mine' }
...
#app-root { max-width: 480px; margin: 0 auto; min-height: 100vh; background: #fff; }
